...
2. The user is still responsible for judging whether a package is appropriate to use. Some releases may still have issues, as indicated by its check results, publicly available at <https://community.r-multiverse.org>.

## Continuous maintainer-driven deployment

Anyone can [contribute](contributors.qmd) packages to Community via GitHub. Each package only needs to be registered once.

Every new [release](https://docs.github.com/en/repositories/releasing-projects-on-github/managing-releases-in-a-repository) then automatically deploys to the repository without needing to go through R-multiverse again. In other words, the package maintainer is in complete control.
